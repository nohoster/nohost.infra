...
if [[ $NODE_TYPE =~ "control0" ]]; then
  ansible all -b -u ubuntu -i "$IP," -m shell -a "curl -sfL https://get.k3s.io | K3S_TOKEN=$K3S_SECRET sh -s - server --cluster-init --flannel-backend=none --cluster-cidr=10.42.0.0/16 --disable-network-policy --disable=metrics-server"
